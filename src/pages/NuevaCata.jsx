...
import { Cata, Muestra, User } from "@/api/entities";
...
export default function NuevaCata() {
  ...
  const [formData, setFormData] = useState({
    ...
    juicio_global: 0,
    puntuacion_visual: 0,
    puntuacion_olfativa: 0,
    puntuacion_gustativa: 0,
    puntuacion_total: 0,
    notas_visuales: "",
    notas_olfativas: "",
    notas_gustativas: "",
  ...
  useEffect(() => {
    const vista = formData.vista_limpidez + formData.vista_color;
    const olfato = formData.olfato_limpidez + formData.olfato_intensidad + formData.olfato_calidad;
    const sabor = formData.sabor_limpio + formData.sabor_intensidad + formData.sabor_persistencia + formData.sabor_calidad;
    const total = vista + olfato + sabor + formData.juicio_global;

    setFormData((prev) => ({
      ...prev,
      puntuacion_visual: vista / 10,
      puntuacion_olfativa: olfato / 30,
      puntuacion_gustativa: sabor / 40,
      puntuacion_total: total
    }));
  }, [
  formData.vista_limpidez, formData.vista_color,
  formData.olfato_limpidez, formData.olfato_intensidad, formData.olfato_calidad,
  formData.sabor_limpio, formData.sabor_intensidad, formData.sabor_persistencia, formData.sabor_calidad,
  formData.juicio_global]
  );

  const createMutation = useMutation({
    mutationFn: (data) => Cata.create(data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['catas'] });
      resetForm();
  ...
  return (
    <div className="min-h-screen p-3 flex justify-start" style={{ backgroundColor: '#F5F5F5' }}>
      <div className="flex-1 max-w-4xl mr-4">
        {/* Formulario */}
        <div className="bg-[#ffffff] p-3 rounded-xl shadow-lg">
          ...
          <div className="mb-2 p-2 rounded-lg" style={{ backgroundColor: '#F5E6FF' }}>
            ...
            <ScoreRow
              label="Intensidad"
              value={formData.sabor_intensidad}
              options={[8, 7, 6, 4, 2]}
              field="sabor_intensidad" />
            <ScoreRow
      ...
      <div className="w-48 space-y-3 self-start pt-2">
        <div className="text-center">
          <h1 className="text-lg font-bold mb-2 text-gray-800">Vinos Tranquilos</h1>
          <p className="text-xs text-gray-600 mb-1">Código</p>
          <p className="text-3xl font-bold text-purple-900">{formData.codigo || '3975'}</p>
        </div>
        ...
        <div className="text-center">
          <p className="text-xs text-gray-500">Tanda {formData.tanda_id || '13'}</p>
        </div>
